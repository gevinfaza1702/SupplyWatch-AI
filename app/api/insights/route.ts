...
export async function GET() {
  ...
  if (!user) {
    return NextResponse.json(
      { error: "Login diperlukan untuk melihat riwayat insight." },
      { status: 401 },
    );
  }
  ...
  if (error) {
    return NextResponse.json(
      { error: "Riwayat insight gagal dimuat.", details: error.message },
      { status: 500 },
    );
  }

  return NextResponse.json({
    insights: ((data ?? []) as AiInsightRow[]).map(mapInsightRow),
  });
}
